...
/**
 * Unit tests for SolarWatchService using MockRestServiceServer to stub external HTTP.
 * Covers: happy path (conversion), city-not-found (404), invalid-tz (400 before HTTP).
 */
class SolarWatchServiceTest {
    ...
    private final String owBase = "https://api.openweathermap.org";
    private final String sunBase = "https://api.sunrise-sunset.org";
    ...
    void getSunTimes_happyPath_convertsToRequestedZone() {
        ...
        // offset must be +02:00 (CEST) in late August
        assertThat(OffsetDateTime.parse(dto.sunrise().toString())
                .getOffset().getTotalSeconds()).isEqualTo(2 * 3600);
    ...
    @Test
    void getSunTimes_invalidTimezone_throws400_beforeAnyHttp() {
        // No expectations: tz parsing fails first, so no HTTP should be executed.
